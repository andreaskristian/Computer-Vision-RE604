...
def load_ground_truth(csv_path: str) -> dict:
    """Baca ground_truth.csv secara robust.

    File hasil edit manual di Excel (apalagi region non-US) sering berantakan:
    delimiter bisa koma atau titik koma, bisa dobel (';;'), bisa ada titik
    koma/koma nyasar di akhir baris, kolom kosong di tengah, dsb. Daripada
    mengandalkan parser CSV standar yang kaku, di sini tiap baris dipecah
    dengan regex berdasarkan SEMUA koma/titik-koma sekaligus, token kosong
    dibuang, lalu token PERTAMA dianggap nama file dan token TERAKHIR
    dianggap teks ground truth.
    """
    import re
    ...
    # Lewati baris header (baris pertama, biasanya "image,ground_truth" atau variannya)
    data_lines = lines[1:] if lines[0].lower().replace(" ", "").startswith("image") else lines

    for line in data_lines:
        tokens = [t.strip().strip('"').strip() for t in re.split(r"[;,]+", line)]
        tokens = [t for t in tokens if t]  # buang token kosong akibat delimiter dobel/nyasar
        if not tokens:
            continue
        image = tokens[0]
        gt = tokens[-1].upper() if len(tokens) > 1 else ""
        if image:
            gt_map[image] = gt
    return gt_map

...
## 0. Kesesuaian dengan Instrumen Asesmen & Justifikasi Penggantian Tool

Tugas ini meminta OCR plat nomor menggunakan VLM yang **"dijalankan melalui
LMStudio"**. Pada praktiknya, LM Studio versi terbaru (0.4.19) **mewajibkan
CPU dengan instruksi AVX2**. Setelah dicoba, laptop yang digunakan memakai
CPU Intel Core i7-3630QM (Ivy Bridge, 2012) yang hanya mendukung AVX (tanpa
AVX2) — LM Studio menampilkan status **"CPU: Incompatible"** pada semua
runtime engine yang tersedia (CUDA maupun CPU), sehingga model tidak dapat
di-load sama sekali (bukti: screenshot pada Settings > Hardware).

Sebagai solusi, digunakan **KoboldCPP** (build `koboldcpp-oldpc.exe`,
backend "Use CPU (Old CPU)") sebagai server lokal pengganti. KoboldCPP:
- Menggunakan format model **GGUF yang sama persis** (didownload melalui
  fitur Search LM Studio, lalu dijalankan lewat KoboldCPP).
- Menyediakan **endpoint API OpenAI-compatible** (`/v1/chat/completions`)
  yang identik strukturnya dengan endpoint Local Server LM Studio — sehingga
  cara integrasi dengan Python (mengirim gambar base64 + prompt, menerima
  teks jawaban) **konsepnya sama persis** seperti referensi resmi LM Studio
  (`lmstudio.ai/docs/python/llm-prediction/image-input`).
- Tetap menjalankan **Visual Language Model (VLM) multimodal**
  (Qwen2-VL-2B-Instruct) secara lokal di komputer sendiri, sesuai maksud
  utama soal: OCR berbasis VLM lokal.

Seluruh alur lain (prompt, format CSV output, rumus CER) mengikuti instruksi
soal secara persis; satu-satunya perbedaan adalah nama aplikasi server
lokal, akibat keterbatasan hardware yang di luar kendali. Poin ini
dijelaskan juga pada video penjelasan proyek (lihat bagian 8).

Kesesuaian tiap poin soal dengan implementasi:

| Poin Soal | Implementasi |
|---|---|
| OCR via VLM multimodal | Qwen2-VL-2B-Instruct (GGUF), lewat KoboldCPP (pengganti LM Studio, lihat justifikasi di atas) |
| Prompt persis sesuai soal | `"What is the license plate number shown in this image? Respond only with the plate number."` (lihat `PROMPT` di `ocr_vlm_lmstudio.py`) |
| Output CSV: `image, ground_truth, prediction, CER_score` | Sesuai persis, lihat `results.csv` |
| Rumus CER = (S+D+I)/N | Diimplementasikan di `cer_utils.py` dengan Levenshtein + backtrace S/D/I |
| Dataset: Indonesian License Plate Recognition (folder test) | `images/test` + `labels/test` (YOLO), plat di-crop per bounding box |
| Source code di GitHub public + README | Lihat bagian 9 |
| Video penjelasan (maks 10 menit) | Lihat bagian 8 |
